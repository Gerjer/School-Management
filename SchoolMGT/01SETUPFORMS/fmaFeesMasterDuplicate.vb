Public Class fmaFeesMasterDuplicate
    ...
    Private Sub displayBatches()
        ...
        SQLEX = "SELECT
                    ...
                    batches.id,
					NAME,
					IFNULL(batches.course_id,0) 'courseID'   
                    FROM
                    batches
                    ...
                    AND course_id = '" & txtCourseID.Text & "' AND batches.school_year = '" & BatchYear & "'
                ...
                If chkbox = True Then
                    ...
                    If clsDBConn.Execute(InsertNewMasterFee) Then
                        ...
                        If resTable.Rows.Count > 0 Then
                            ...
                            'Dim getParticulars As String = "SELECT name,amount,mode_payments,IFNULL(account_code,0)'account_code'  FROM finance_fee_particulars"
                            'getParticulars += " WHERE finance_fee_category_id='" & FeeCatID & "'"


                            Dim getParticulars As String = "SELECT name,amount,mode_payments,CASE WHEN account_code IS NULL THEN 0 "
                            getParticulars += " WHEN account_code = '' THEN 0"
                            getParticulars += " ELSE account_code END AS 'account_code'"
                            getParticulars += " FROM finance_fee_particulars WHERE finance_fee_category_id= '" & FeeCatID & "'"
                            ...
                            If particularTbl.Rows.Count > 0 Then
                                For pNctr As Integer = 0 To particularTbl.Rows.Count - 1
                                    ...
                                    Dim account_code As Integer = particularTbl.Rows(pNctr).Item("account_code").ToString

                                    If Not IsDBNull(particularTbl.Rows(pNctr).Item("amount")) Then
                                        amount = CDbl(particularTbl.Rows(pNctr).Item("amount").ToString)
                                    End If
                                    Dim insetNewParticularFee As String = "INSERT INTO finance_fee_particulars(name,description,amount,finance_fee_category_id,is_deleted,mode_payments,account_code)"
                                    ...
                                    insetNewParticularFee += String.Format("'{0}','{1}','{2}')", "0", mode_payments, account_code)
