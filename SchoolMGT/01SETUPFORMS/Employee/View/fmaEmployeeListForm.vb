        ...
        If Me.Tag = 1 Then
            ...
            FROM
                ...
                where person.first_name LIKE '%" & txtEmpName.Text & "%' OR
                      ...
                      employees.status_type_id = '" & status_type_id & "' and employees.end_time is null AND
                      person_type_id = 1
                ;"
        ...
        Else
			...
			`person`
			 ...
			 where person.first_name LIKE '%" & txtEmpName.Text & "%' OR
                        ...
                        /*AND person.status_type_id = '" & status_type_id & "'*/ 
                        AND person.end_time IS NULL 
							)A
               WHERE person_type_id = 2 and status_type_id = '" & status_type_id & "';"
        End If
...
#End Region
    ...
    Private Sub btnSearchCondition_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearchCondition.Click
        ...
        new_display_master_record_student()
    ...
    Private Sub new_display_master_record_student()


        Dim dt As DataTable = querryDatatable(_dt_master_student, "status_type_id", status_type_id)
        dt_PersonList = dt
        gvPersonList.Columns.Clear()

        getDesignGridControl(dt)
        gvPersonList.Columns("status_type_id").Visible = False
        gvPersonList.Columns("prsnID").Visible = False
        txtPageSize.Properties.TextEditStyle = DevExpress.XtraEditors.Controls.TextEditStyles.DisableTextEditor
        Class1.Innitial(gcPersonList, txtDisplayPageNo, txtPageSize)
    ...
    Private Function querryMasterStudent(dt_master_student As DataTable, Column As String, value As String) As DataTable
        Dim result = dt_master_student.Clone()

        For Each row As DataRow In From row1 As DataRow In dt_master_student.Rows Where (row1(Column) = (CInt(value)))
            result.ImportRow(row)
        Next

        'Dim rowArray As DataRow() = dt_master_student.[Select](" " & Column & " = " & status_type_id & "")
...
#End Region
    ...
    Private Sub rdbActive_CheckedChanged(sender As Object, e As EventArgs) Handles rdbActive.CheckedChanged
        Dim status As RadioButton = sender
        If status.Checked = True Then
            status_type_id = "1"
        End If
    End Sub

    Private Sub rdbInactive_CheckedChanged(sender As Object, e As EventArgs) Handles rdbInactive.CheckedChanged
        Dim status As RadioButton = sender
        If status.Checked = True Then
            status_type_id = "2"
        End If
    ...
    Private Sub rdbGraduated_CheckedChanged(sender As Object, e As EventArgs) Handles rdbGraduated.CheckedChanged
        Dim status As RadioButton = sender
        If status.Checked = True Then
            status_type_id = "3"
        End If
    ...
    Private Sub rbtUnderGrad_CheckedChanged(sender As Object, e As EventArgs) Handles rbtUnderGrad.CheckedChanged
        Dim status As RadioButton = sender
        If status.Checked = True Then
            status_type_id = "4"
        End If
    ...
    Private Sub rdbStop_CheckedChanged(sender As Object, e As EventArgs) Handles rdbStop.CheckedChanged
        Dim status As RadioButton = sender
        If status.Checked = True Then
            status_type_id = "5"
        End If
    ...
    Private Sub ActiveToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ActiveToolStripMenuItem.Click
        ...
        Dim msg As String = ""
        msg = "Are your sure...? " & vbCrLf
        msg += "You want to change the status into ACTIVE"
        If MessageBox.Show(msg, "Pleas verify....", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
            DataSource(String.Format("Update person SET status_type_id = 1 WHERE person_id = '" & personID & "' "))
            MessageBox.Show("Successfully Updated!!")
            UpdateMasterStudent(personID, "1")
            new_display_master_record_student()
        End If

    End Sub

    Private Sub GraduatedToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles GraduatedToolStripMenuItem.Click

        Dim msg As String = ""
        msg = "Are your sure...? " & vbCrLf
        msg += "You want to change the status into GRADUATED"
        If MessageBox.Show(msg, "Pleas verify....", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
            DataSource(String.Format("Update person SET status_type_id = 3 WHERE person_id = '" & personID & "'"))
            MessageBox.Show("Successfully Updated!!")
            UpdateMasterStudent(personID, "3")
            new_display_master_record_student()
        End If
    End Sub

    Private Sub UnderGraduatedToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles UnderGraduatedToolStripMenuItem.Click
        Dim msg As String = ""
        msg = "Are your sure...? " & vbCrLf
        msg += "You want to change the status into UNDER GRADUATE"
        If MessageBox.Show(msg, "Pleas verify....", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
            DataSource(String.Format("Update person SET status_type_id = 4 WHERE person_id = '" & personID & "'"))
            MessageBox.Show("Successfully Updated!!")
            UpdateMasterStudent(personID, "4")
            new_display_master_record_student()
        End If
    End Sub

    Private Sub UpdateMasterStudent(personID As Integer, status_type_id As String)

        For Each row As DataRow In From row1 As DataRow In _dt_master_student.Rows Where (row1("prsnID") = (personID))
            row.SetField("status_type_id", CInt(status_type_id))
            row.AcceptChanges()
        Next
    ...
    Private Sub StoppedToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles StoppedToolStripMenuItem.Click
        Dim msg As String = ""
        msg = "Are your sure...? " & vbCrLf
        msg += "You want to change the status into STOPPED"
        If MessageBox.Show(msg, "Pleas verify....", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
            DataSource(String.Format("Update person SET status_type_id = 5 WHERE person_id = '" & personID & "'"))
            MessageBox.Show("Successfully Updated!!")
            UpdateMasterStudent(personID, "5")
            new_display_master_record_student()
        End If
    End Sub
    ...
    Private Sub InactiveToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles InactiveToolStripMenuItem.Click
        Dim msg As String = ""
        msg = "Are your sure...? " & vbCrLf
        msg += "You want to change the status into INACTIVE"
        If MessageBox.Show(msg, "Pleas verify....", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
            DataSource(String.Format("Update person SET status_type_id = 2 WHERE person_id = '" & personID & "'"))
            MessageBox.Show("Successfully Updated!!")
            UpdateMasterStudent(personID, "2")
            new_display_master_record_student()
        End If
    End Sub

    Private Sub txtPageSize_SelectedIndexChanged(sender As Object, e As EventArgs) Handles txtPageSize.SelectedIndexChanged
        Cursor.Current = Cursors.WaitCursor
