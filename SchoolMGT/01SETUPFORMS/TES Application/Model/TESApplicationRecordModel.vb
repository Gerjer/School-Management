    ...
    Friend Sub Insert(TES As TESApplication, operation As String)
        ...
        Try
            ...
            With TES
                ...
                sql &= String.Format(" from_year = '{0}',", .indigenous)
                sql &= String.Format(" year_level = '{0}',", .year_level)
                sql &= String.Format(" student_id = '{0}',", .student_id)
                sql &= String.Format(" scholarship_grant_id = '{0}'",.scholar_id )
...
"
    ...
    Friend Function getStudentList(academic_year As String, scholar_name As String) As Object
        Dim sql As String = ""
        sql = "SELECT
person.person_id 'ID',
...
scholarship.id 'scholrID',
students.year_level

FROM
person
INNER JOIN students ON person.person_id = students.person_id AND students.is_enrolled = 1
INNER JOIN courses ON students.course_id = courses.id
INNER JOIN person_address ON person.person_id = person_address.person_id
left JOIN scholarship ON students.id = scholarship.student_id
WHERE
person.status_type_id = 1 AND
person.end_time IS NULL AND
students.status_type_id = 1 AND
students.end_time IS NULL AND
person_address.address_type_id = 1 AND
students.academice_year = '" & academic_year & "' AND
students.scholarshipgrant = '" & scholar_name & "' AND
scholarship.id IS NULL 
