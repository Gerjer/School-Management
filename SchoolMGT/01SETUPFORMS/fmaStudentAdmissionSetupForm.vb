...
--  where `students_details`.`status` = 1 and `students_details`.end_time is null        
    ...
    Private Sub getScholarShipGrant()
        Dim SQLEX As String = "SELECT code, name, fullDeduct from scholarship_grant"
        SQLEX += " order by code"

        Dim MeData As DataTable
        MeData = clsDBConn.ExecQuery(SQLEX)

        txtGrant.DataSource = MeData

        txtGrant.ValueMember = "code"
        txtGrant.DisplayMember = "name"

        txtGrant.SelectedIndex = -1
        txtIsFullDeduct.Text = ""
...
"))
    ...
    Private Sub txtGrant_SelectedIndexChanged(sender As Object, e As EventArgs) Handles txtGrant.SelectedIndexChanged
        ...
        Try
            Dim drv As DataRowView = DirectCast(txtGrant.SelectedItem, DataRowView)
            Me.txtIsFullDeduct.Text = drv.Item("fullDeduct").ToString
        Catch ex As Exception
            Me.txtIsFullDeduct.Text = ""
        End Try


        If Me.txtIsFullDeduct.Text = "YES" Then
            cxbxGrant.Checked = True
        Else
            cxbxGrant.Checked = False
        End If
